...
const TIER_1 = ['portugal', 'france', 'italy', 'spain', 'united kingdom', 'sweden', 'canada', 'brazil', 'germany', 'austria', 'switzerland'];
...
function matches(origin: string, country: string): boolean {
  const escaped = country.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  return new RegExp(`(^|[^a-z])${escaped}([^a-z]|$)`).test(origin);
}

export function originTier(origin: string | null | undefined): OriginTier | null {
  if (!origin) return null;
  const o = origin.toLowerCase().trim();
  if (TIER_1.some((c) => matches(o, c))) return 1;
  if (TIER_2.some((c) => matches(o, c))) return 2;
  if (TIER_3.some((c) => matches(o, c))) return 3;
